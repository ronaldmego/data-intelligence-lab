...
CHAIN_NOTES = {
    "product fee": "what the customer signed",
...
def main() -> None:
    ...
    ap.add_argument("--per-balance", type=float, default=None,
                    help="override the marginal cost per 1k of balance from the cost model")
    ...
    if args.per_balance is not None:
        cost_model = cost_model.with_per_balance(args.per_balance)
    ...
    print(f"  contribution      mean {result.mean_contribution:,.2f} at {result.cost_model.per_balance:.2f}/1k; "
          f"{sensitivity.top_product} falls below {sensitivity.cheapest_plan} at "
          f"{sensitivity.inversion_per_balance:.2f}/1k")
    ...
    print(f"  value axis        {axis.share_moved:.1%} changed band; "
          f"{len(axis.still_plans)} of {len(axis.by_product)} tariffs moved nobody; "
          f"{axis.concentration:.0%} of movers are {axis.worst_plan.product_id}")
