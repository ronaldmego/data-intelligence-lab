...
def usage_chart(evidence) -> str:
    ...
    rows = [evidence.overall, *evidence.by_product]
    row_h = 34.0
    top = PAD_T + 40
    ...
    body = [_text(18, 20, "Do customers who carry more balance generate more revenue?",
                  anchor="start", size=13)]
    ...
    for i, link in enumerate(rows):
        ...
        body.append(f'<circle cx="{centre:.1f}" cy="{y + 10:.1f}" r="{5.5 if aggregate else 4}" '
                    f'fill="{colour}"/>')
        label = "all products together" if aggregate else f"within {link.scope}"
...
def axis_chart(axis) -> str:
    ...
    for i, product in enumerate(rows):
        y = top + i * row_h
        width = _linear(product.share_moved, 0.0, top_value, x0, x1) - x0
        colour = ACCENT if product.straddles else INK
        if product.moved:
            body.append(f'<rect x="{x0}" y="{y:.1f}" width="{max(1.5, width):.1f}" height="18" '
                        f'fill="{colour}" fill-opacity="0.80"/>')
        body.append(_text(x0 - 10, y + 8, f"{product.product_id} · {product.monthly_fee:.0f}/month",
                          anchor="end", size=11, weight="bold"))
        body.append(_text(x0 - 10, y + 22,
                          "straddles a band threshold" if product.straddles else "never crosses a threshold",
                          anchor="end", size=9, opacity=0.85))
        if product.moved:
            body.append(_text(x0 + width + 8, y + 13, f"{product.share_moved:.1%}", anchor="start",
                              size=11, weight="bold", fill=colour))
        else:
            body.append(_text(x0 + 8, y + 13, "not one customer", anchor="start", size=10,
                              fill=POSITIVE))
        body.append(_text(x1 + 20, y + 13, f"{product.customers:,}", anchor="start", size=9, opacity=0.8))
