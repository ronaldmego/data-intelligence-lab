...
* **the product ladder** — "upgrade to M" only means something relative to the product
  the customer is on, and the shared ``products`` table numbers tiers across
  families (a prepaid M and a credit S are both tier 2). Rank has to be taken
...
_TYPE_OBJECTIVE = {"discount": "retention", "upgrade": "upsell", "limit_increase": "crosssell"}
...
class ProductLadder:
    ...
    for eligibility: prepaid M and credit S are both tier 2, so "is this an
    ...
    rank: dict[str, int]                     # product_id -> 1-based rank in its family
    family: dict[str, str]                   # product_id -> family
    fee: dict[str, float]                    # product_id -> monthly fee
    by_family_rank: dict[tuple[str, int], str]  # (family, rank) -> product_id

    @classmethod
    def build(cls, tables: Tables) -> ProductLadder:
        ...
        for fam, products in families.items():
            for position, product in enumerate(sorted(products, key=lambda p: _int(p["tier"])), start=1):
                product_id = product["product_id"]
                rank[product_id] = position
                family[product_id] = fam
                fee[product_id] = _float(product["monthly_fee"])
                by_family_rank[(fam, position)] = product_id
        return cls(rank=rank, family=family, fee=fee, by_family_rank=by_family_rank)

    def target_plan(self, product_id: str, target_rank: int) -> str | None:
        """The plan an upgrade offer would move this customer to, if any."""
        return self.by_family_rank.get((self.family[product_id], target_rank))
...
class Offer:
    ...
    offer_id: str
    name: str
    type: str                    # discount | limit_increase | upgrade
    value: float                 # discount share, or thousands of limit, depending on type
