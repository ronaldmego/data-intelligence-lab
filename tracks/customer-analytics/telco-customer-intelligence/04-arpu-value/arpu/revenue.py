...
* **the variance split**: how much of the spread in monthly invoices is between
  plans rather than within them;
* **the usage link**: whether customers who use more generate more revenue —
  asked once over the whole base, and then again *inside each plan*, which is
...
from .data import Plan, Tables, _f
...
def correlation_se(n: int) -> float:
    ...
    Reported alongside every within-plan correlation because at twenty thousand
...
def split_revenue(tables: Tables, plans: dict[str, Plan], plan_of: dict[str, str],
    ...
    for row in tables["billing"]:
        if row["period_month"] > cutoff:
            continue
        plan = plans[plan_of[row["customer_id"]]]
        amount = _f(row["amount_billed"])
        amounts.append(amount)
        fees.append(plan.monthly_fee)
        by_plan.setdefault(plan.plan_id, []).append(amount)

    n = len(amounts)
    grand = mean(amounts)
    total_variance = sum((a - grand) ** 2 for a in amounts)
    between = sum(len(v) * (mean(v) - grand) ** 2 for v in by_plan.values())
...
class UsageLink:
    ...
    scope: str            # "all plans" or a plan_id
    n: int
    r: float
...
@dataclass(frozen=True)
class UsageEvidence:
    """The aggregate reading, and the same reading inside each plan."""

    overall: UsageLink
    by_plan: list[UsageLink]

    @property
    def largest_within(self) -> UsageLink:
        return max(self.by_plan, key=lambda link: abs(link.r))

    @property
    def ratio(self) -> float:
        """How many times bigger the aggregate correlation is than the biggest
        within-plan one. The number that says the aggregate is composition."""
    ...
    def within_readable(self) -> list[UsageLink]:
        return [link for link in self.by_plan if link.readable]


def usage_link(tables: Tables, plans: dict[str, Plan], plan_of: dict[str, str],
    ...
    rather than about which plan they bought.
    """
    usage = {
        (row["customer_id"], row["period_month"]): _f(row["data_gb_used"])
        for row in tables["usage_monthly"]
        if row["period_month"] <= cutoff
    }
    ...
    for row in tables["billing"]:
        ...
        if key not in usage:
            continue
        plan = plans[plan_of[row["customer_id"]]]
        overage.append(_f(row["amount_billed"]) - plan.monthly_fee)
        gigabytes.append(usage[key])
        plan_ids.append(plan.plan_id)

    by_plan: list[UsageLink] = []
    for plan_id in sorted(plans, key=lambda p: plans[p].monthly_fee):
        idx = [i for i, p in enumerate(plan_ids) if p == plan_id]
        if len(idx) < 3:
            continue
        by_plan.append(UsageLink(
            scope=plan_id,
            n=len(idx),
            r=correlation([overage[i] for i in idx], [gigabytes[i] for i in idx]),
        ))

    return UsageEvidence(
        overall=UsageLink(scope="all plans", n=len(overage), r=correlation(overage, gigabytes)),
        by_plan=by_plan,
    )
...
@dataclass(frozen=True)
class PlanNoise:
    """How far a plan's measured ARPU wanders from the price the customer signed."""
...
def estimator_quality(measured: dict[str, float], plan_of: dict[str, str],
                      plans: dict[str, Plan], months_averaged: int,
    ...
    per_plan: list[PlanNoise] = []
    for plan_id in sorted(plans, key=lambda p: plans[p].monthly_fee):
        values = [v for cid, v in measured.items() if plan_of.get(cid) == plan_id]
        if len(values) < 2:
            continue
    ...
    return EstimatorQuality(
        ...
        per_plan=per_plan,
        distinct_true_values=len({plans[p].monthly_fee for p in plans}),
