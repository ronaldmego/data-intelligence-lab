...
def load_plans(tables: Tables) -> dict[str, Plan]:
    return {
        row["plan_id"]: Plan(
            plan_id=row["plan_id"],
            family=row["family"],
            monthly_fee=_f(row["monthly_fee"]),
            data_gb=_f(row["data_gb"]),
...
def plan_of(tables: Tables) -> dict[str, str]:
    return {row["customer_id"]: row["current_plan_id"] for row in tables["customers"]}
