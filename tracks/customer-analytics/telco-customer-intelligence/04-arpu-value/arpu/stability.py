...
class AxisStability:
    ...
    def straddling_plans(self) -> list[PlanMovement]:
        return [p for p in self.by_plan if p.straddles]
    ...
    def still_plans(self) -> list[PlanMovement]:
        ...
        return [p for p in self.by_plan if p.moved == 0]

    @property
    def worst_plan(self) -> PlanMovement:
        return max(self.by_plan, key=lambda p: p.share_moved)


def measure_axis(before: dict[str, float], after: dict[str, float], plan_of: dict[str, str],
    ...
    by_plan: list[PlanMovement] = []
    for plan_id in sorted({plan_of[c] for c in shared}, key=lambda p: fees[p]):
        members = [c for c in shared if plan_of[c] == plan_id]
        values = [after[c] for c in members]
        measured_mean = mean(values)
        nearest = min(cuts_after, key=lambda cut: abs(cut - measured_mean))
        by_plan.append(PlanMovement(
            plan_id=plan_id,
            monthly_fee=fees[plan_id],
