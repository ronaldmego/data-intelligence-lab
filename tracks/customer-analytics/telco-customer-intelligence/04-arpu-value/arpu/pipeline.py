...
class CaseResult:
    ...
    plan_economics: list[PlanContribution] = field(default_factory=list)
    ...
    def value_chain(self) -> list[tuple[str, float]]:
        ...
        fee = mean(self.plans[p].monthly_fee for p in [pr.plan_id for pr in self.profiles])
    ...
    **rate** comes from a year, because a payment-failure rate estimated on three
    ...
    for cid in population.customer_ids:
        ...
        profiles.append(ServiceProfile(
            customer_id=cid,
            plan_id=assignment[cid],
            billed=billed,
            collected=billed * rate,
            data_gb=trailing_mean(usage[cid], "data_gb_used", cutoff, REVENUE_WINDOW_MONTHS),
...
def run_case(
    ...
    plans = load_plans(tables)
    assignment = plan_of(tables)
    ...
    result = CaseResult(
        churn=churn, population=population, plans=plans, profiles=profiles,
    ...
    # --- 3. the level -----------------------------------------------------
    result.split, result.variance = split_revenue(tables, plans, assignment, cutoff)
    result.usage = usage_link(tables, plans, assignment, cutoff)
    ...
    result.plan_economics = plan_contributions(profiles, plans, cost_model)
    result.sensitivity = sensitivity(profiles, plans, cost_model)
    ...
    result.axis = measure_axis(
        ...
        after=after,
        plan_of=assignment,
        fees={pid: plan.monthly_fee for pid, plan in plans.items()},
