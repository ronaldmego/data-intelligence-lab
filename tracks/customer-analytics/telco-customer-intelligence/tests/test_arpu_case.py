...
def test_measured_arpu_never_falls_below_the_tariff(result):
    ...
    for profile in result.profiles:
        assert profile.billed >= result.plans[profile.plan_id].monthly_fee - 1e-9
...
def test_cost_model_row_changes_the_contribution(result, tmp_path):
    ...
    assert "0.40,currency" in text, "the per-GB row moved; update this test with it"
...
def test_break_even_costs_are_solved_not_sampled(result):
    ...
    assert sensitivity.inversion_per_gb is not None

    model = result.cost_model.with_per_gb(sensitivity.inversion_per_gb)
    rows = {p.plan_id: p for p in plan_contributions(result.profiles, result.plans, model)}
    assert rows[sensitivity.top_plan].contribution == pytest.approx(
        rows[sensitivity.cheapest_plan].contribution, abs=1e-6
    )

    # …and it is a genuine crossing: the order is opposite either side of it.
    below = result.cost_model.with_per_gb(sensitivity.inversion_per_gb - 0.02)
    above = result.cost_model.with_per_gb(sensitivity.inversion_per_gb + 0.02)
    lower = {p.plan_id: p.contribution for p in plan_contributions(result.profiles, result.plans, below)}
    upper = {p.plan_id: p.contribution for p in plan_contributions(result.profiles, result.plans, above)}
    assert lower[sensitivity.top_plan] > lower[sensitivity.cheapest_plan]
    assert upper[sensitivity.top_plan] < upper[sensitivity.cheapest_plan]
...
def test_straddling_is_counted_not_inferred_from_distance(result):
    ...
    """
    for plan in result.axis.by_plan:
        if not plan.straddles:
            assert plan.moved == 0
        assert (plan.below_cut > 0 and plan.above_cut > 0) == plan.straddles
        assert plan.below_cut + plan.above_cut == plan.customers
...
def test_axis_movement_is_concentrated_in_the_straddling_tariffs(result):
    ...
    assert len(result.axis.still_plans) >= len(result.axis.by_plan) - 2
...
def test_axis_uses_case_01_band_machinery_on_both_cutoffs():
    ...
    axis = measure_axis(
        ...
        plan_of={c: "P" for c in before}, fees={"P": 1.0},
